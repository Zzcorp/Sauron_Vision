...
WIRING = {
    ...
    "scraper_sec":          {"layer": "ingest", "writes": ["InstitutionalFiling"], "feeds": ["pipeline_opportunity_scanner"],
                             "note": "13F rows are written with a null instrument, and the only consumer filters on instrument — so those rows are stored and never read."},
    "scraper_cot":          {"layer": "ingest", "writes": [], "feeds": ["pipeline_opportunity_scanner"],
                             "note": "The persist call exists but is unreachable: the CFTC parser yields nothing, so it returns before writing."},
    ...
    "scraper_finviz":       {"layer": "ingest", "writes": [], "feeds": [],
                             "note": "No model exists to hold a screener hit, and no page would show one."},
    ...
    "pipeline_event_engine":        {"layer": "eye", "writes": [], "feeds": ["execute_bots"],
                                     "note": "A registered component whose Celery wrapper does not exist. Nothing consults its switch, so toggling it changes nothing."},
